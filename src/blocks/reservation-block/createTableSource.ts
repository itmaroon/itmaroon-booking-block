...
export const renderTableFromTableSource = (
	...
	if ( ! tableRoot || ! Array.isArray( tableSource ) ) {
		return;
	}
	...
	for ( const row of tableSource ) {
		...
		for ( const cell of row.cells ) {
			...
			//属性を記録する
			if ( cell.attributes ) {
				Object.entries( cell.attributes ).forEach(
					( [ key, value ] ) => {
...
export const slotInfoCalendar = (
	...
	( slots ?? [] ).forEach( ( row: SlotRow ) => {
		...
		if ( row.status === 'open' ) {
			...
			if ( ! row.is_booked ) {
				tempDailyStats[ day ][ time ].avail += Number(
					row.capacity.max
				);
			}
		}
